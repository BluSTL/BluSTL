function [F,P] = getMILP_no_loop_robust(phi,t0,k,ts,var,M)
    ...
    switch (phi.type)
        ...
        case 'not'
            [Frest,Prest] = getMILP_no_loop_robust(phi.phi,t0,k,ts, var,M);
            [Fnot, Pnot] = not(Prest);
            F = [F, Fnot, Frest];
            P = Pnot;

        case 'or'
            [Fdis1,Pdis1] = getMILP_no_loop_robust(phi.phi1,t0,k,ts, var,M);
            [Fdis2,Pdis2] = getMILP_no_loop_robust(phi.phi2,t0,k,ts, var,M);
            [For, Por] = or([Pdis1;Pdis2],M);
            F = [F, For, Fdis1, Fdis2];
            P = Por;

        case 'and'
            [Fcon1,Pcon1] = getMILP_no_loop_robust(phi.phi1,t0,k,ts, var,M);
            [Fcon2,Pcon2] = getMILP_no_loop_robust(phi.phi2,t0,k,ts, var,M);
            [Fand, Pand] = and([Pcon1;Pcon2],M);
            F = [F, Fand, Fcon1, Fcon2];
            P = Pand;

        case '=>'
            [Fant,Pant] = getMILP_no_loop_robust(phi.phi1,t0,k, ts,var,M);
            [Fcons,Pcons] = getMILP_no_loop_robust(phi.phi2,t0,k,ts, var,M);
        ...
        case 'until'
            [Fp,Pp] = getMILP_no_loop_robust(phi.phi1,t0,k, ts, var,M);
            [Fq,Pq] = getMILP_no_loop_robust(phi.phi2,t0,k, ts, var,M);
